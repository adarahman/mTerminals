...
# disconnected from paths.RUNTIME_DIR/CACHE_DIR (where every other
# cache/log lives) and resolved relative to the process's cwd. Now
# shares the same runtime/ tree via config.py; ANGELONE_CACHE_DIR still
# overrides it for anyone relying on the old path.
CACHE_DIR = Path(settings.instrument_cache_dir)
...
def get_available_expiries(underlying: str, refresh: bool = False) -> list[str]:
    """Future expiries from the public instrument master in display format."""
    global _default_resolver
    if refresh or _default_resolver is None:
        _default_resolver = InstrumentResolver().load(force_refresh=refresh)
    requested = (underlying or "").strip().upper()
    if not requested:
        return []
    from brokers.symbol_names import canonicalize_underlying

    canonical = canonicalize_underlying(requested, _default_resolver._chain_index) or requested
    today = date.today()
    parsed = []
    for raw in _default_resolver.get_available_expiries(canonical):
        try:
            expiry = datetime.strptime(raw, "%d%b%Y")
        except ValueError:
            continue
        if expiry.date() >= today:
            parsed.append(expiry)
    return [expiry.strftime("%d-%b-%Y") for expiry in sorted(parsed)]
...
def get_futures_contract(
    ...
    *,
    ...
    """Resolve a FUTIDX/FUTSTK row from the public instrument master."""
    global _default_resolver
    if refresh or _default_resolver is None:
        _default_resolver = InstrumentResolver().load(force_refresh=refresh)
    requested = (underlying or "").strip().upper()
    ...
    known = {
        row.get("name")
        for row in _default_resolver._instruments
        if row.get("name") and row.get("instrumenttype") in _FNO_FUT_TYPES
    }
    canonical = canonicalize_underlying(requested, known) or requested
    candidates = []
    for row in _default_resolver._instruments:
        if (
            row.get("exch_seg") != exchange
            or row.get("name") != canonical
            or row.get("instrumenttype") not in _FNO_FUT_TYPES
        ):
            continue
        try:
            parsed_expiry = datetime.strptime(row.get("expiry", ""), "%d%b%Y")
        except (TypeError, ValueError):
            continue
    ...
    if expiry:
        try:
            target = datetime.strptime(expiry, "%d-%b-%Y").strftime("%d%b%Y").upper()
        except ValueError:
            return None
        return next((row for row, _ in candidates if row.get("expiry") == target), None)
    today = datetime.combine(date.today(), datetime.min.time())
    live = [(row, exp) for row, exp in candidates if exp >= today] or candidates
    index = min({"NEAR": 0, "NEXT": 1, "FAR": 2}.get(which.upper(), 0), len(live) - 1)
    return live[index][0]
