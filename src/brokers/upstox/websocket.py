...
`from upstox_client import ...` (bare, not `from brokers.upstox.client
...
def _import_official_sdk():
    ...
    saved_path = sys.path[:]
    saved_module = sys.modules.get("brokers.upstox.client")
    try:
        ...
        if saved_module is not None:
            sys.modules["brokers.upstox.client"] = saved_module
...
from brokers.upstox.client import _session as _own_session
...
class UpstoxTickStream:
    ...
    def connect(self):
        ...
        from brokers.upstox.execution import ensure_session as _ensure_upstox_session
