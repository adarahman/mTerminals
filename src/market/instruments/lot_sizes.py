...
_live_resolver: Callable[[str], int] | None = None


def configure_lot_size_resolver(resolver: Callable[[str], int] | None) -> None:
    """Supply the outer broker adapter used for live instrument metadata."""
    global _live_resolver
    _live_resolver = resolver
...
def get_lot_size(symbol: str, default: int | None = 65) -> int:
    ...
    try:
        if _live_resolver is None:
            raise RuntimeError("live lot-size resolver is not configured")
        lot = _live_resolver(sym)
