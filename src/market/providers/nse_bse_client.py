"""
market/providers/nse_bse_client.py
=============
...
def get_or_create_sheet(wb, sheet_name):
    ...
    if sheet_name not in [s.name for s in wb.sheets]:
        logger.info(f"'{sheet_name}' not found — creating it.")
...
def ensure_session(force: bool = False) -> requests.Session:
    ...
    for url, extra in warm_up_urls:
        try:
            ...
            logger.error(f"[ensure_session] warm-up failed for {url}: {e}")
...
def nse_request(url: str, referer: str = "", _retried: bool = False, _conn_retries: int = 2):
    ...
    try:
        ...
        if _conn_retries > 0:
            ...
            logger.error(f"[nse_request] request error: {e} — retrying in {wait:.1f}s ({_conn_retries} left)")
            time.sleep(wait)
            return nse_request(url, referer, _retried=_retried, _conn_retries=_conn_retries - 1)
        logger.error(f"[nse_request] request error: {e} — out of retries")
        return None

    if resp.status_code in (401, 403) and not _retried:
        logger.info(f"[nse_request] HTTP {resp.status_code} — re-warming session and retrying")
        ensure_session(force=True)
        return nse_request(url, referer, _retried=True, _conn_retries=_conn_retries)

    if resp.status_code != 200:
        logger.info(f"[nse_request] HTTP {resp.status_code} for {url}")
        return None
    ...
    with ThreadPoolExecutor(max_workers=min(len(indices), 7)) as ex:
        ...
        for future in as_completed(futures):
            try:
                ...
                logger.error(f"[fetch_all_indices] thread error for {futures[future]}: {e}")
...
def get_unified_market_data(df_idx: pd.DataFrame | None = None):
    ...
    if df_idx is not None and not df_idx.empty and not ticker_payload:
        logger.warning(f"[get_unified_market_data] WARNING: 0 ticker pills matched "
...
def fetch_bse_index_quote(symbol: str) -> dict | None:
    ...
    try:
        ...
        logger.error(f"[fetch_bse_index_quote] parse error for {symbol}: {e}")
        return None

    if not ltp:
        logger.info(f"[fetch_bse_index_quote] {symbol}: got response but LTP/CurrRate/PrevClose "
...
def fetch_option_chain(symbol: str, expiry: str,
    ...
    logger.info(f"[market_api] Fetching option chain — {symbol_upper} {expiry}")
    ...
    if "records" not in payload:
        if "expiryDates" in payload or "data" in payload:
            logger.info("[market_api] Adapting flat API-v3 response → records envelope")
            payload = {
                "records": {
    ...
    rec = payload["records"]
    logger.info(f"[market_api] Option chain OK — "
...
def write_all_indices_sheet(wb, df: pd.DataFrame, sheet_name: str = "AllIndices") -> None:
    ...
    if df.empty:
        logger.info("No index data to write.")
        return
    ...
    logger.info(f"[+] Wrote {len(df)} rows → '{sheet_name}'.")
...
def write_futures_sheet(wb, df: pd.DataFrame, sheet_name: str = "FuturesData") -> None:
    ws = get_or_create_sheet(wb, sheet_name)
    if df.empty:
        logger.info("No futures data to write.")
        return
    ws.range("A1").options(index=False).value = df
    logger.info(f"[+] Wrote {len(df)} rows → '{sheet_name}'.")
...
def bse_request(url: str, params: dict | None = None):
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.error(f"[-] BSE HTTP {resp.status_code} for {url}")
            return None
        return resp.json()
    except Exception as e:
...
def fetch_bse_json_options(expiry_str: str = "02 Jul 2026",
    ...
    if not data or "Table" not in data:
        logger.error("[-] BSE options: no 'Table' key in response")
        return None, 0.0
    ...
    if not rows:
        logger.error("[-] BSE options: no valid rows")
        return None, 0.0

    df = pd.DataFrame(rows).sort_values("Strike").reset_index(drop=True)
    logger.info(f"[+] BSE options: {len(df)} strikes  spot={spot}")
    _numeric_cols = [
...
def fetch_bse_futures(expiry_str: str = None,
    ...
    logger.info(f"[*] BSE futures — Expiry: {expiry_str}  scrip_cd: {scrip_cd}")
    ...
    if not table:
        logger.error("[-] BSE futures: empty Table")
        return None
    ...
    logger.info(f"[+] BSE futures: {len(df)} rows")
    return df
...
def dump_bse_to_excel(expiry_str: str = "02 Jul 2026",
    ...
    if df is None or df.empty:
        logger.error("[-] BSE Excel dump aborted: empty DataFrame.")
        return

    logger.info(f"[*] BSE spot={spot}  Writing {len(df)} rows → '{sheet_name}'…")
    ...
    try:
        ...
        ws.clear_contents()
        ws.range("A1").options(index=False).value = df
        logger.info(f"[+] BSE Excel dump complete → '{sheet_name}'.")
    except Exception as e:
        logger.error(f"[-] BSE Excel bridge failure: {e}")
...
def fetch_all_indices_snapshot() -> pd.DataFrame:
    ...
    data = nse_request(url, referer="https://www.nseindia.com/")
    if not data:
        logger.info("[fetch_all_indices_snapshot] API returned no data")
        return pd.DataFrame()
    ...
    df = pd.DataFrame(rows)
    logger.info(f"[fetch_all_indices_snapshot] {len(df)} indices fetched "
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("NSE — allIndices snapshot")
    snap_df = fetch_all_indices_snapshot()
    if not snap_df.empty:
        logger.info(snap_df[["key", "index", "last", "percentChange"]].to_string(index=False))
    logger.info(f"  VIX (from snapshot) = {get_vix_from_snapshot(snap_df)}")
