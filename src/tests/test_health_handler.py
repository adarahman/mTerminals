...
def test_health_is_ok_for_fresh_open_market_snapshot(ws_server_live, monkeypatch):
    ...
    monkeypatch.setattr(module._HEALTH_SNAPSHOT, "_market_session_status", lambda _: "OPEN")
    ...
    health = module._HEALTH_SNAPSHOT.build(now)
...
def test_closed_market_without_snapshot_is_idle_not_failed(ws_server_live, monkeypatch):
    ...
    monkeypatch.setattr(
        module._HEALTH_SNAPSHOT,
        "_market_session_status",
        lambda _: "MARKET_CLOSED",
    )
...
def test_health_handler_uses_service_status_code(ws_server_live, monkeypatch):
    module = ws_server_live
    monkeypatch.setattr(
        module.runtime_state.HTTP_ROUTE_HANDLERS,
        "_health_snapshot",
        lambda: {"status": "degraded", "reasons": ["feed stale"]},
    )
