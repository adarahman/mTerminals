...
def _ensure_history_loaded(log_path):
    ...
    if _HISTORY_MEM.loaded_for(log_path):
        return
...
def read_last_json_snapshot(symbol, log_path=JSON_HISTORY_LOG_PATH):
    ...
    logger.info(f"Found previous snapshot from {last_ts} ({len(prev_poll)} rows) for {symbol} — using it for deltas.")
    return prev_poll
...
def append_json_history(
    ...
    if _HISTORY_MEM.flush_due(flush_interval_seconds):
...
def flush_history_to_disk(log_path=JSON_HISTORY_LOG_PATH):
    ...
    try:
        ...
        _HISTORY_MEM.mark_flushed()
    except Exception as e:
        logger.warning(f"Could not write {log_path} ({e}). Snapshot was not persisted.")
