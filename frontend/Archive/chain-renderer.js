...
ChainView.prototype.onExpiryChange = function(selectedExpiry) {
  if(!_data) return;
  // Phase 5 (event-bus.js): announce the switch before doing any of the
  // existing work below, which is untouched — additive only. (Previously
  // added only to the unused chain-views.js copy of this file — ported
  // here to the file that's actually loaded.)
...
ChainView.prototype._rerenderChainPanels = function() {
  ...
  // ── 6. IV Surface ─────────────────────────────────────────────────────────
  // Alerts card — same outerHTML-diff treatment as the Greeks alerts/ATM
  // Greeks cards above, so an expiry switch or live tick reflects new
  // skew/IV-rank immediately.
  //
  // ROOT CAUSE FIX: this block used to rebuild the FULL per-strike IV bar
  // table directly via `#sec-iv .section-card` on every tick — a leftover
  // from before IV Surface was split into buildIvAlertsHtml() (summary,
  // main dashboard) + buildIvSurfaceHtml()/renderIvSurfaceModal() (full
  // table, modal, both in chain-template.js/chain-renderer.js already).
  // buildIvAlertsHtml()'s own card also matches `.section-card` inside
  // `#sec-iv`, so this was silently overwriting the summary card with the
  // old full table on the very next tick after every rebuild — the main
  // dashboard never actually stopped showing the full table.
  const ivAlertsEl = document.getElementById("iv-alerts-card");
  if(ivAlertsEl){
    const freshIvAlerts = app.chain.buildIvAlertsHtml(_data, chain, atm);
    if(ivAlertsEl.dataset.lastHtml !== freshIvAlerts){
      ivAlertsEl.outerHTML = freshIvAlerts;
      const fresh = document.getElementById("iv-alerts-card");
      if(fresh) fresh.dataset.lastHtml = freshIvAlerts;
    }
  }
  // Full IV surface — keeps the modal current on every tick, same as
  // Greeks/GEX below, regardless of whether it's open. renderIvSurfaceModal()
  // already existed but was never actually called from here, so the modal's
  // content div only ever got populated once, on open.
  if (window.renderIvSurfaceModal) renderIvSurfaceModal();

  // ── 7. Greeks & GEX panels ───────────────────────────────────────────────
  renderGreeksGex(_grkView);

  // ── 7b. FII / DII participant OI modal ──────────────────────────────────
  // Same reasoning as IV Surface above — keeps the full comparison table
  // current inside its modal on every tick instead of only on open.
  if (window.renderFiiDiiModal) renderFiiDiiModal(_data);
  ...
  // Phase 5 (event-bus.js): announce that every chain-derived panel above
  // just finished repainting off _data. Ported here from the unused
  // chain-views.js copy, which was never actually loaded.
  if (window.eventBus) window.eventBus.emit('chain:update', _data);
};
