...
class PriceChartEngine {
  ...
  render(forceResize){
    ...
    // Phase 5 (event-bus.js): announce a completed redraw. render() is
    // called very often (every tick, every hover frame, every zoom/pan,
    // every toolbar toggle) — this fires on all of them identically,
    // same as it always did, just with one more side-effect appended.
    if (window.eventBus) window.eventBus.emit('chart:refresh', { range: this.settings.range });
  }
}
