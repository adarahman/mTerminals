...
function mergeDeltaIntoChain(changedRows) {
  ...
  if (SPOT == null || isNaN(SPOT)) {
    ...
    Logger.warn('oi-flow', 'delta arrived before SPOT was known, skipping merge.');
    return false;
  }
  if (!Array.isArray(CHAIN) || CHAIN.length === 0) {
    // No base chain to merge into yet either — same reasoning.
    Logger.warn('oi-flow', 'delta arrived before CHAIN was seeded, skipping merge.');
    return false;
  }
...
function adaptLiveMessage(msg) {
  ...
  // NOT YET WIRED UP: delta ticks are currently dropped here without
  // touching chart state. To finish this, replace the return below with:
  //
  ...
  if (!Array.isArray(strikesArr)) {
    Logger.warn('oi-flow', 'connected, but payload.chain missing.', Object.keys(body));
...
function connectWS() {
  ...
  ws.onclose = () => {
    ...
    setTimeout(connectWS, Config.ws.reconnectDelayMs);
...
function startMock() {
  ...
  seedMock();
  render();
  mockTimer = setInterval(() => { jitterMock(); render(); }, Config.refresh.mockJitterMs);
}
