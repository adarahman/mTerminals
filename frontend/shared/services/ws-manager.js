...
class WSManager {
  ...
  connect(url) {
    ...
    try { socket = new WebSocket(this.url); }
      ...
      // error event will ever fire for this attempt, so without an explicit
      // reconnect() here the connection just gives up forever after one
      // failed attempt instead of retrying like the onclose path below does.
      this.reconnect();
      return;
    }
    this.ws = socket;
