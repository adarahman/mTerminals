...
class UiControls {
  ...
  // Tier-3 targets (e.g. iv-hv-skew-detail-card) are <details class="card">
  // elements, collapsed by default — scrolling to a closed disclosure with
  // no visible content defeats the point of "jump to section". Force it
  // open first (covers both the target itself being a <details> and it
  // being nested inside one).
  const detailsEl = el.tagName==='DETAILS' ? el : el.closest('details');
  if(detailsEl) detailsEl.open = true;

  // Scroll manually instead of scrollIntoView({block:'start'}) — that
  // aligns the target's top edge with the viewport's top edge, which is
  // exactly where the sticky .top-bar (position:sticky, ~44px+padding,
  // z-index:997) sits, so the section's own heading was landing right
  // underneath it. --panel-top is already the live-measured height of
  // everything sticky-stacked above the scrolling content (see
  // updateStickyOffsets() above), so reuse it here as the scroll offset
  // instead of a second hardcoded guess that could drift out of sync
  // with it.
  const offsetPx = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--panel-top')) || 60;
  const EXTRA_GAP = 8;
  const targetY = el.getBoundingClientRect().top + window.scrollY - offsetPx - EXTRA_GAP;
