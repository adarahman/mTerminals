...
//
// tickFill() also lives here (merged in from the now-deleted chain-utils.js,
// which existed as a separate file for exactly this one method) — it's the
// striped-gradient helper used by the OI/ΔOI/velocity bars below and in the
// right-hand analytics panel, i.e. every call site was already in this file.
//
// NOTE: declared as a plain function, not ChainDenseView.prototype.tickFill
// (which is how it lived in the old chain-utils.js). Every call site below
// (RightPanelView.prototype.renderRightPanel, .rpBar) calls it bare —
// tickFill(clr), never this.tickFill(clr) — and RightPanelView is a
// different class than ChainDenseView, so a bare call could never have
// resolved to a ChainDenseView.prototype method in the first place. This
// was a pre-existing bug in chain-utils.js (a ReferenceError the moment
// renderRightPanel()/rpBar() ran), not something introduced by this merge.
// tickFill takes no `this`, so a plain function is also the more correct
// shape: nothing here is genuinely ChainDenseView-specific.
// ============================================================

function tickFill(clr) {
...
RightPanelView.prototype.renderRightPanel = function(rows) {
    ...
    // arpClr removed — was using --ce/--pe (option-side identity colors
    // per theme.css), which is a semantic misuse for a sign indicator.
    // Now uses the shared signColor() from formatters.js (--pos/--neg).
    ...
    <div class="rp-box">
      ...
      <div class="arp-row"><span class="arp-key">Net OI</span><div class="arp-val"><span class="arp-num" style="color:${signColor(netOI)};">${netOI >= 0 ? "+" : ""}${fmt(netOI)}</span><div class="arp-bar" style="width:${arpBarW(netOI, netAbsMax)}px;background-image:${tickFill(signColor(netOI))};"></div></div></div>
      <div class="arp-row"><span class="arp-key">Chg OI</span><div class="arp-val"><span class="arp-num" style="color:${signColor(netDOI)};">${netDOI >= 0 ? "+" : ""}${fmt(netDOI)}</span><div class="arp-bar" style="width:${arpBarW(netDOI, netAbsMax)}px;background-image:${tickFill(signColor(netDOI))};"></div></div></div>
      <div class="arp-row"><span class="arp-key">Vel OI</span><div class="arp-val"><span class="arp-num" style="color:${signColor(netVel)};">${netVel >= 0 ? "+" : ""}${fmt(netVel)}</span><div class="arp-bar" style="width:${arpBarW(netVel, netAbsMax)}px;background-image:${tickFill(signColor(netVel))};"></div></div></div>
