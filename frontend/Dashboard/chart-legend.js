(function() {
  ...
  //
  // A second canvas (#greeksChart-modal) lives in the click-to-expand
  // modal (see DashboardPro.html / openGreeksChartModal in panels-views.js)
  // — same "expand" treatment Strategy Payoff and the Net GEX chart
  // already get, just Chart.js-based instead of a manual 2D canvas draw,
  // so it needs its own Chart.js instance rather than sharing the inline
  // one (a single Chart.js instance can only ever bind to one canvas).
  // charts is keyed by canvas id so both instances can be created lazily
  // and kept current independently, whether or not the modal is open —
  // same guarantee the manually-drawn charts give via their
  // _draw*OnCanvas(id) helpers being called once per canvas id every
  // render pass.
  const charts = {}; // canvasId -> Chart.js instance

  function ensureGreeksChart(canvasId) {
    canvasId = canvasId || 'greeksChart';
    const canvasEl = document.getElementById(canvasId);
    if (!canvasEl) return null;
    const existing = charts[canvasId];
    if (existing && existing.canvas === canvasEl) return existing;
  ...
  window.updateGreeksMoneynessChart = function(payload, force) {
    ...
    // whether or not the modal is currently open). Both stay in lockstep
    // on every tick, same guarantee _drawPayoffOnCanvas/_drawGexOnCanvas
    // give the Strategy Payoff / Net GEX expand modals — so opening
    // either one never shows a stale or empty chart.
    const mainChart = ensureGreeksChart('greeksChart');
    const modalChart = ensureGreeksChart('greeksChart-modal');
    if (!mainChart && !modalChart) return;
    ...
    const labels = order.map((k, i) => labelFor(k, i));
    ...
    const gammaN = norm(gamma), thetaN = norm(theta), vegaN = norm(vega);

    [mainChart, modalChart].forEach(chart => {
      if (!chart) return;
      chart.data.labels = labels;
      chart.data.datasets[0].data = delta;
      chart.data.datasets[1].data = gammaN;
      chart.data.datasets[2].data = thetaN;
      chart.data.datasets[3].data = vegaN;
