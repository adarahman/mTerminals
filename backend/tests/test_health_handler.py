...
def test_health_is_ok_for_fresh_open_market_snapshot(ws_server_live, monkeypatch):
    ...
    monkeypatch.setattr(module, "_market_session_status", lambda _: "OPEN")
    monkeypatch.setattr(module, "LAST_PAYLOAD", {"symbol": "NIFTY"})
    monkeypatch.setattr(module, "LAST_PAYLOAD_AT", now - timedelta(seconds=2))
    ...
    assert health["websocket"]["connectedClients"] == len(module.CONNECTED)
...
def test_health_degrades_when_open_market_snapshot_is_stale(ws_server_live, monkeypatch):
    ...
    now = datetime(2026, 8, 7, 10, 0, tzinfo=timezone.utc)
    monkeypatch.setattr(module, "_market_session_status", lambda _: "OPEN")
    monkeypatch.setattr(module, "POLL_SECONDS", 5)
    monkeypatch.setattr(module, "LAST_PAYLOAD", {"symbol": "NIFTY"})
    monkeypatch.setattr(module, "LAST_PAYLOAD_AT", now - timedelta(seconds=30))
...
def test_closed_market_without_snapshot_is_idle_not_failed(ws_server_live, monkeypatch):
    ...
    monkeypatch.setattr(module, "_market_session_status", lambda _: "MARKET_CLOSED")
    monkeypatch.setattr(module, "LAST_PAYLOAD", None)
    monkeypatch.setattr(module, "LAST_PAYLOAD_AT", None)
