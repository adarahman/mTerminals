...
def _restore_runtime_state():
    ...
    provider + ws_server_live's DATA_SOURCE + option_chain_json's
    USE_SMARTAPI) after every test, so mutations from switch tests can't
    leak into other tests sharing the session-scoped ws_server_live module."""
    prev_md = md.get_active_provider()
    ws = sys.modules.get("ws_server_live")
    prev_ds = ws.DATA_SOURCE if ws is not None else None
    ocj = sys.modules.get("option_chain_json")
    prev_sa = ocj.USE_SMARTAPI if ocj is not None else None
    yield
    try:
        ...
        pass
    if ws is not None:
        ws.DATA_SOURCE = prev_ds
    if ocj is not None:
        ocj.USE_SMARTAPI = prev_sa
...
def test_config_rejects_nse_bse_as_execution_broker(tmp_path):
    """config.py must fail fast at import when EXECUTION_BROKER=NSE_BSE.

    Runs in an isolated temp copy of config.py/paths.py so the repo's own
...
def test_canonicalize_underlying_maps_company_names(ws_server_live):
    ...
    from brokers.smartapi_client import _canonical_underlying, list_expiries
...
def test_resolves_full_company_name_via_upstox_master_reference():
    ...
    from brokers.upstox_client import (
...
def test_upstox_find_equity_token_resolves_full_company_name(ws_server_live):
    ...
    from brokers.upstox_client import find_equity_token
...
def test_upstox_canonical_name_maps_ticker_to_full_name(ws_server_live):
    ...
    from brokers.upstox_client import _canonical_name
...
def test_runtime_switch_without_restart(ws_server_live, monkeypatch):
    ...
    assert ws_server_live.DATA_SOURCE == "NSE_BSE"
    ...
    assert ws_server_live.DATA_SOURCE == "UPSTOX"
...
def test_switch_rejects_unknown_and_same_source_is_noop(ws_server_live, monkeypatch):
    ...
    before = ws_server_live.DATA_SOURCE
...
def test_switch_symbol_unquotes_stale_encoded_symbol(ws_server_live, monkeypatch):
    ...
    orig_symbol = ws_server_live.SYMBOL
    orig_expiry = ws_server_live.EXPIRY
    try:
        ws_server_live.switch_symbol("zydus%20lifesciences%20ltd")
        assert ws_server_live.SYMBOL == "ZYDUS LIFESCIENCES LTD"
        ws_server_live.switch_symbol("BANKNIFTY")  # clean input still works
        assert ws_server_live.SYMBOL == "BANKNIFTY"
    finally:
        ws_server_live.SYMBOL = orig_symbol
        ws_server_live.EXPIRY = orig_expiry
...
def test_canon_symbol_resolves_full_name_for_lot_and_engine_keys(monkeypatch):
    ...
    import option_chain_json as ocj

    assert ocj._canon_symbol("ADANI ENERGY SOLUTION LTD") == "ADANIENSOL"
    assert ocj._canon_symbol("NIFTY") == "NIFTY"  # indices pass through
    assert ocj._canon_symbol("ADANIENSOL") == "ADANIENSOL"  # idempotent
...
def test_index_quotes_provider_aware(ws_server_live, monkeypatch):
    ...
    ws_server_live.DATA_SOURCE = "NSE_BSE"
    assert ws_server_live.fetch_index_quotes_smartapi_sync() == {}

    ws_server_live.DATA_SOURCE = "KITE"
...
def test_pipeline_gate_activates_public_nse_path(ws_server_live):
    ...
    ws_server_live.DATA_SOURCE = "NSE_BSE"
    ws_server_live._configure_pipeline_globals("NIFTY")
    assert option_chain_json.USE_SMARTAPI is False  # public NSE/BSE chain path

    ws_server_live.DATA_SOURCE = "UPSTOX"
    ws_server_live._configure_pipeline_globals("NIFTY")
    assert option_chain_json.USE_SMARTAPI is True  # broker REST chain path
