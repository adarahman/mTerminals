...
_CACHE_DIR = Path(os.getenv("UPSTOX_CACHE_DIR", os.path.join(os.path.expanduser("~"), ".upstox_cache")))
...
def build_login_url(state: str, api_key: str = None, redirect_uri: str = None) -> str:
    ...
    if not params["client_id"] or not params["redirect_uri"]:
        raise UpstoxError("UPSTOX_API_KEY / UPSTOX_REDIRECT_URI not set (and none passed explicitly)")
    return f"{API_BASE}/v2/login/authorization/dialog?{urlencode(params)}"


def exchange_code_for_token(code: str, api_key: str = None, api_secret: str = None,
...
class UpstoxSession:
    ...
    def request(self, method: str, url: str, *, params=None, json_body=None,
                form_body=None, max_retries=3) -> dict:
        ...
        raise UpstoxError(f"{method} {url} failed after {max_retries} attempts: {last_exc}")
...
def _load_instrument_dump(scope: str = "complete", force_refresh: bool = False,
                           max_age_hours: float = 20.0) -> list:
    ...
    url = f"{ASSETS_BASE}/{scope.lower()}.json.gz"
    response = requests.get(url, timeout=60)
    if response.status_code != 200:
        if dump_path.exists():
            logger.warning("[upstox_client] instrument master refresh failed (%s), using stale cache",
    ...
    logger.info("[upstox_client] cached %d instrument rows for scope=%s", len(rows), scope)
    return rows
...
def _row_symbol(row: dict) -> str:
    ...
    return row.get("trading_symbol") or row.get("tradingsymbol") or row.get("symbol") or ""
...
def list_expiries(underlying: str, exchange: str = "NFO") -> list:
    ...
    scope = "NSE" if exchange.upper() in ("NFO", "NSE") else exchange.upper()
...
def find_option_token(underlying: str, expiry: str, strike, opt_type: str,
...
def find_equity_token(symbol: str, exchange: str = "NSE") -> Optional[dict]:
    ...
    rows = _load_instrument_dump(exchange.upper())
    symbol = symbol.upper()
    for row in rows:
        if row.get("instrument_type") == "EQ" and _row_symbol(row).upper() == symbol:
            return {"instrument_key": row.get("instrument_key"), "trading_symbol": _row_symbol(row)}
    return None
...
def get_ltp(instrument_keys) -> dict:
    ...
    payload = _session.request("GET", f"{API_BASE}/v3/market-quote/ltp",
...
def get_quotes(instrument_keys) -> dict:
    ...
    if isinstance(instrument_keys, (list, tuple, set)):
        instrument_keys = ",".join(instrument_keys)
    payload = _session.request("GET", f"{API_BASE}/v2/market-quote/quotes",
                                params={"instrument_key": instrument_keys})
    return payload.get("data", {})
...
def get_spot_quote(underlying: str) -> Optional[dict]:
    ...
    key = index_instrument_key(underlying) or (find_equity_token(underlying) or {}).get("instrument_key")
    if not key:
        return None
    data = get_ltp(key)
...
def get_historical_candles(instrument_key: str, unit: str, interval: int,
                            from_date: str, to_date: str = None) -> list:
...
def get_atm_chain(underlying: str, expiry: str, strikes_around_atm: int = 10,
                   exchange: str = "NFO") -> Optional[dict]:
    ...
    strikes = sorted({float(r["strike_price"]) for r in legs if r.get("strike_price") is not None})
    ...
    if atm_strike is None:
        return None
    window = {atm_strike + i * step for i in range(-strikes_around_atm, strikes_around_atm + 1)} if step else {atm_strike}
    ...
    return {"underlying": underlying_u, "spot": spot_price, "atm_strike": atm_strike,
            "expiry": expiry, "rows": out_rows}
...
def modify_order(order_id: str, *, quantity: int = None, order_type: str = None,
                  price: float = None, trigger_price: float = None,
                  validity: str = None, disclosed_quantity: int = None) -> dict:
    ...
    for key, val in (("quantity", quantity), ("order_type", order_type), ("price", price),
...
def cancel_order(order_id: str) -> dict:
    return _session.request("DELETE", f"{HFT_BASE}/v2/order/cancel", params={"order_id": order_id})
...
def get_positions() -> list:
    ...
    for row in payload.get("data") or []:
        item = dict(row)
        item.setdefault("tradingsymbol", row.get("trading_symbol"))
        item.setdefault("pnl", (row.get("pnl") if row.get("pnl") is not None
