...
ml/inference.py
───────────────
Runtime/serving half of the former virtual_oi_estimator.py. Fills the
NSE 3-minute OI update cooling gap using ML-predicted next-snapshot OI
deltas, computed from each poll cycle's own real deltas (ce_vol_delta,
pe_vol_delta, ce_oi_delta_lag, pe_oi_delta_lag, ce_iv_delta, pe_iv_delta,
minutes_since_last) — the exact schema ml.training trains on. One CE
and one PE HuberRegressor pipeline are trained independently
(production_oi_pipeline_ce.pkl / _pe.pkl); the coordinator routes each
strike/side to its own pipeline.
...
Training/MLOps side lives in ml/training.py — see evaluate_and_deploy_pipeline().
"""
...
class VirtualOIEstimator:
    ...
    def on_nse_refresh(self, confirmed_oi: float):
        ...
        if drift_ratio > 0.30:
            logger.info(f"[VirtualOI] {self.symbol} | Reconciliation drift {drift_ratio*100:.1f}% "
...
def load_virtual_oi_coordinator(model_registry_dir="model_registry") -> "VirtualOICoordinator | None":
    ...
    if not models:
        logger.info("[VirtualOI] No production model found (looked for "
    ...
    if missing_side:
        logger.info(f"[VirtualOI] Loaded production model(s): {found}. "
