...
# Where normalized EOD datasets get persisted. Anchored to PROJECT_ROOT (same
# pattern as paths.RUNTIME_DIR/CACHE_DIR) rather than a bare cwd-relative
# path — this used to be Path("data/eod"), which resolved differently
# depending on whether the process was launched from the project root or
# from inside backend/, silently splitting reads/writes across two
# different directories. Override via nse_eod_fetch.DATA_DIR = Path(...)
# before calling fetch_all_eod(save=True), or just pass an explicit out_dir.
DATA_DIR = Path(PROJECT_ROOT) / "data" / "eod"
